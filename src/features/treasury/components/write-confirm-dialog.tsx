...
export function WriteConfirmDialog({
  ...
  const [acknowledgedWrite, setAcknowledgedWrite] =
    useState<StagedWrite | null>(null)

  const blocks =
    pending?.warnings.filter((warning) => warning.severity === 'block') ?? []
    ...
    >
      <DialogContent>
              ...
              <li
                className={
                  warning.severity === 'block'
                    ? 'border-destructive/40 bg-destructive/10 text-destructive rounded-md border p-3 text-xs leading-relaxed font-medium'
                    : 'border-warning/40 bg-warning/10 text-warning rounded-md border p-3 text-xs leading-relaxed'
                }
                key={warning.message}
              >
                {warning.message}
        ...
        {pending && blocks.length > 0 ? (
          ...
          <label className="text-muted-foreground flex cursor-pointer items-start gap-2 text-xs">
            <input
              ...
              onChange={(event) =>
                setAcknowledgedWrite(event.target.checked ? pending : null)
              }
              type="checkbox"
            />
            <span>
        ...
        <DialogFooter>
          ...
          <Button
            disabled={
              !pending || submitting || (blocks.length > 0 && !acknowledged)
            }
            onClick={onSubmit}
          >
