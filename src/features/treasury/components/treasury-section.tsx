...
export function TreasurySection({
  ...
  const treasuryLocked = snapshot
    ? snapshot.status.paused || snapshot.status.frozen || !snapshot.status.initialized
    : false;
      ...
      {fallback ?? (snapshot === null ? (
        ...
        <div className="grid grid-cols-4 gap-3 max-xl:grid-cols-2 max-sm:grid-cols-1">
          <Metric
            ...
            value={snapshot.status.initialized ? "Initialized" : "Not initialized"}
          />
          <Metric
...
export function ApprovalPlanSection({
  ...
  const fallback = renderQueryFallback({
    connected,
    disconnectedDescription: "Connect a wallet to load context rules from the smart account.",
    error,
    ...
    <div className="grid gap-4 rounded-lg border bg-card p-5 shadow-sm">
      ...
      <div className="grid gap-2">
        {fallback ?? (rules.length === 0 ? (
          ...
          rules.map((rule) => (
            <div
              className="grid grid-cols-[88px_1fr] gap-x-3 gap-y-1 rounded-md border bg-background p-3 text-sm"
              key={rule.id}
            >
