...
export function SignersSection({
      ...
      const titles =
        functionName === 'remove_signer'
          ...
          : functionName === 'add_context_rule'
            ? {
                confirmedTitle: 'Rule created',
                submittedTitle: 'Rule creation submitted',
              }
            : functionName === 'remove_context_rule'
              ? {
                  confirmedTitle: 'Rule removed',
                  submittedTitle: 'Rule removal submitted',
                }
              : {
  ...
  // and click.
  async function stageSignerRemoval(
    rule: ContextRule,
    address: string,
    index: number
  ) {
    try {
      ...
      stageMutation.mutate({
        operation: removeSignerOperation({
  ...
  function stageRuleRemoval(rule: ContextRule) {
    ...
    stageMutation.mutate({
      operation: removeContextRuleOperation({
        contextRuleId: rule.id,
        contracts,
      }),
      targetRule: rule,
    ...
    >
      ...
      {rules.length > 0 ? (
        <div className="border-border bg-secondary mb-4 rounded-lg border p-3 text-sm">
          {(() => {
            const { weakestUnanimousRule, policyGatedRuleIds } =
              computeWeakestRule(rules)
            return (
              <>
                {weakestUnanimousRule ? (
                  <p>
                    <strong>Weakest path to full control:</strong>{' '}
                    {weakestUnanimousRule.requiredSigners} signature
                    {weakestUnanimousRule.requiredSigners === 1 ? '' : 's'}{' '}
                    (Rule {weakestUnanimousRule.id} ·{' '}
                    {weakestUnanimousRule.name})
                  </p>
                ) : (
                  <p>
                    <strong>Weakest path to full control:</strong> every rule
                    has a policy attached — exact threshold not readable here.
                  </p>
                )}
                {policyGatedRuleIds.length > 0 ? (
                  <p className="text-muted-foreground mt-1 text-xs">
                    {policyGatedRuleIds.length} rule
                    {policyGatedRuleIds.length === 1 ? '' : 's'} (
                    {policyGatedRuleIds.join(', ')}) also carr
                    {policyGatedRuleIds.length === 1 ? 'ies' : 'y'} a policy —
      ...
      {!wallet.address ? (
        ...
        <>
                    ...
                    {rule.signerAddresses.map((address, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {!canWrite ? null : block ? (
                          ...
                          ) : pendingRemoval?.ruleId === rule.id &&
                            pendingRemoval.index === index ? (
                            ...
                            <Button
                              onClick={() =>
                                setPendingRemoval({ ruleId: rule.id, index })
                              }
                            >
          ...
          {!canWrite ? (
            <div className="border-border bg-secondary mt-6 rounded-lg border p-4 text-sm">
              <strong>
                This wallet cannot change this treasury&apos;s signers.
              </strong>
              <p className="text-muted-foreground mt-1 text-xs">
                {truncateAddress(wallet.address)} is not a delegated signer on
                any context rule above, so the SmartAccount rejects every write
                from it. Connect a wallet registered on{' '}
            ...
            <>
              <div className="border-border mt-6 rounded-lg border border-dashed p-4">
                <strong className="text-sm">
                  Create a new, independent rule
                </strong>
                <p className="text-muted-foreground mt-1 text-xs">
                  Not a helper or a limited role: this signer gets full,
                  independent power over this treasury, equal to every other
                  rule&apos;s signers — because any one satisfied rule
                  authorizes anything. This treasury becomes only as secure as
                  this new rule. Use this only when you want a genuinely
                  separate, co-equal path to control (e.g. a second team with
                  its own key) — not to add a backup signer to an existing rule
                  (use &ldquo;Add signer&rdquo; below for that).
                </p>
              ...
              <div className="mt-6 grid gap-3">
                <strong className="text-sm">
                  Add signer to an existing rule
                </strong>
                <p className="text-muted-foreground text-xs">
                  Gives this signer whatever role that rule already grants — a
                  co-signer, not a new independent power. To grant full,
                  independent control instead, use &ldquo;Create a new,
                  independent rule&rdquo; above.
                </p>
                <div className="grid gap-2">
                  ...
                  >
                    {rules.length === 0 ? (
                      <option value="">No context rule is loaded yet.</option>
                    ) : null}
                    {rules.map((rule) => (
                ...
                <Button
                  onClick={() => {
                    try {
                      ...
                      if (!targetRule)
                        throw new Error('No context rule is available.')
                      stageMutation.mutate({
                ...
                <p className="text-muted-foreground text-xs">
                  Simulation cannot confirm authorization. The wallet signature
                  is what proves it.
                </p>
              </div>
            </>
