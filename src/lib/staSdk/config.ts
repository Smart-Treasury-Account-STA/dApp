/**
 * Network configuration, vendored from the smart-contracts repo's
 * `sdk/packages/core/src/config.ts` ("sta-sdk") — the actual upstream SDK
 * this module is a faithful port of.
 *
 * This is a distinct, self-consistent module from this dApp's own
 * operational config (`@/config`, `@/lib/env`), which is env-var-driven
 * and is what the running app actually uses. `staSdk/` mirrors the
 * upstream SDK's own hardcoded-per-network approach instead, so this
 * stays a genuine 1:1 port rather than a re-interpretation — treat it as
 * the vendored reference implementation, not the app's live config.
 *
