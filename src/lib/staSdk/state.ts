 ...
 * recovery-state, ported from the smart-contracts repo's
 * `sdk/packages/core/src/state.ts` ("sta-sdk"). Every function here is a
 * read-only simulation (no signature, no submission).
 *
 * Adapted from upstream: the original builds each read on a generated
 * per-contract `Client` (`smartAccountClient(net).status()`, etc.), which
 * this dApp can't depend on directly -- see `auth.ts`'s module doc comment
 * for why. Reads here go straight through `rpc.Server.simulateTransaction`
 * instead, the same low-level pattern this dApp's own `stellarClient.ts`
 * already uses and has verified correct, live. One real signature
 * difference from upstream follows from this: every function here takes
 * an explicit `sourceAddress` (a real, funded account to build the
 * simulation transaction from -- `getAccount` fails for one that doesn't
 * exist on-chain), where upstream's generated-client calls don't expose
 * one. Every type below (`AccountStatus`, `ContextRule`, `ScheduledIntent`,
 ...
 * vendored `stellar-accounts` OZ crate for `ContextRule`), not
 * reconstructed from observed JSON shapes.
 */
