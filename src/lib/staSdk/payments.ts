/**
 * Transaction-preparation helpers: prepare -> simulate -> approve, ported
 * from the smart-contracts repo's `sdk/packages/core/src/payments.ts`
 * ("sta-sdk"). Each function returns a prepared, auth-entry-attached
 * `Transaction` -- simulated (fee/resource estimation) with the custom
 * `smart_account` `AuthPayload` entries already present, ready for the
 * caller to sign the envelope with a fee-paying account and submit (see
 * `signAndSubmit` below). None of these functions touch a wallet directly
 * -- signing is always an injected `Keypair | SigningCallback` (see
 * `auth.ts`), so a browser dApp can swap in a connected wallet's
 * `signAuthEntry` without changing anything here.
 *
 * Adapted from upstream: the original's `PrepareOptions.smartAccountClient`
 * (a generated-bindings client, used only for `.spec.funcArgsToScVals(...)`
 * to encode each call's args) is dropped -- see `auth.ts`'s module doc
 * comment for why this dApp can't depend on those generated bindings
 * directly. Args are hand-encoded here instead (`transferArgs`,
 * `splitArgs`, `scheduledIntentScVal`, below), against the same contract
 * signatures, the same way this dApp's own `stellarClient.ts` already
 * does for these same calls.
 */
import {
  Account,
  Address,
...
import { buildExecutorAuthEntry, buildInvocation, buildSmartAccountAuthEntries } from "@/lib/staSdk/auth";
import type { NetworkConfig } from "@/lib/staSdk/config";
