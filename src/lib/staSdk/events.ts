/**
 * Typed event parsing, ported from the smart-contracts repo's
 * `sdk/packages/core/src/events.ts` ("sta-sdk") — vendored rather than
 * depended on directly because that SDK's other modules pull in generated
 * per-contract bindings pinned to `@stellar/stellar-sdk ^14.5.0`, and this
 * dApp runs `^16.2.0`; this one file has no such dependency (only
 * `@stellar/stellar-sdk`'s `xdr`/`scValToNative`), so it ports cleanly on
 * its own. Keep in sync by hand if the source contracts' `#[contractevent]`
 * definitions change — there is no automated link between the two repos.
 *
