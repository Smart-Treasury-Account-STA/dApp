 ...
 * construction, ported from the smart-contracts repo's
 * `sdk/packages/core/src/auth.ts` ("sta-sdk").
 *
 * One deliberate adaptation from upstream: the original's
 * `buildSmartAccountAuthEntries` takes a generated contract client's
 * `Spec` (from `@stellar/stellar-sdk/contract`) and calls
 * `spec.nativeToScVal(...)` to encode the `AuthPayload` struct against the
 * contract's own on-chain spec. That `Spec` comes from the generated
 * per-contract bindings (`@sta/smart_account-bindings`), which are pinned
 * to `@stellar/stellar-sdk ^14.5.0` and not published anywhere this dApp
 * (on `^16.2.0`) could safely depend on -- two majors of the same classes
 * (`Address`, `xdr.ScVal`, ...) loaded at once risk `instanceof`
 * mismatches between them. This port hand-encodes the identical
 * `AuthPayload` struct shape directly instead (`smartAccountAuthPayloadScVal`
 * below) -- the same approach this dApp's own `stellarClient.ts` already
 * used and had verified correct, live, before this module existed. The
 * struct shape itself (`{ signers: Map<Signer, Bytes>, context_rule_ids:
 * Vec<u32> }`) is unchanged from upstream.
 *
