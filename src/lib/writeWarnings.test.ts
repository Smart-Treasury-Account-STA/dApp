...
describe('collectWriteWarnings — bump_version against the relayer queue', () => {
  it('counts queued intents that the bump would strand', () => {
    const warnings = collectWriteWarnings(
      bumpVersionOperation({ nextVersion: 2 }),
      {
        ...context,
  ...
  it('stays quiet when nothing is stranded', () => {
    ...
    expect(
      warnings.filter((w: WriteWarning) => w.message.includes('#2006'))
    ).toEqual([])
  })
})
...
describe('collectWriteWarnings — destructive policy changes', () => {
  it('warns that disabling transfer stops every payment', () => {
    ...
    expect(
      warnings.some((w: WriteWarning) => /stop every payment/i.test(w.message))
    ).toBe(true)
  })
  ...
  it('warns when an asset is being disabled', () => {
    ...
    expect(warnings.some((w: WriteWarning) => /disable/i.test(w.message))).toBe(
      true
    )
  })
})
...
describe('collectWriteWarnings — authority mismatch', () => {
  it('warns when the connected key is not the treasury owner (source-account op)', () => {
    ...
    expect(
      warnings.some((w: WriteWarning) =>
        /not the treasury owner/i.test(w.message)
      )
    ).toBe(true)
  })
...
describe('collectWriteWarnings — the rule this wallet will authorize through', () => {
  ...
  it('stays quiet when this wallet has a rule it can satisfy alone', () => {
    ...
    expect(
      warnings.some((w: WriteWarning) => /one signature/i.test(w.message))
    ).toBe(false)
  })

  it('warns rather than blocks when a policy might still accept one signature', () => {
    const allRules = [
      rule({ id: 0, signerCount: 3, signerAddresses: [OWNER], policyCount: 1 }),
    ]
...
describe('collectWriteWarnings — removing a context rule', () => {
  it('warns naming the rule and how many signers lose access', () => {
    const targetRule = rule({
      ...
      name: 'payroll',
      signerCount: 2,
      signerAddresses: ['GEXISTING', 'GOTHER'],
    })
    const warnings = collectWriteWarnings(
      removeContextRuleOperation({ contextRuleId: 0 }),
      {
        ...context,
        targetRule,
      }
    ...
    expect(
      warnings.some((w: WriteWarning) => /2 of its signers/i.test(w.message))
    ).toBe(true)
  })
  ...
  it("adds a self-removal warning when the connected wallet is one of the rule's own signers", () => {
    ...
    expect(
      warnings.some((w: WriteWarning) =>
        /connected wallet.*(own signers|lose access)/i.test(w.message)
      )
    ).toBe(true)
  })
  ...
  it("does not add a self-removal warning when the connected wallet is not one of the rule's signers", () => {
    ...
    expect(
      warnings.some((w: WriteWarning) =>
        /lose access|own signers/i.test(w.message)
      )
    ).toBe(false)
  })
})
