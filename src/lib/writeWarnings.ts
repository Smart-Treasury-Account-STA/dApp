...
export function collectWriteWarnings(
  ...
  if (operation.functionName === 'bump_version') {
    ...
    const stranded = context.pinnedVersions.filter(
  ...
  if (
    operation.functionName === 'set_operation_allowed' &&
    /Disable/.test(operation.summary)
  ) {
    warnings.push({
      severity: 'warn',
      message:
  ...
  if (
    operation.functionName === 'set_asset_rule' &&
    /Disable/.test(operation.summary)
  ) {
    warnings.push({
      severity: 'warn',
      message:
        'Disable this asset and every transfer of it stops, including queued intents.',
    })
  }
  ...
  if (operation.functionName === 'add_context_rule' && context.allRules) {
    const { requiredSigners: existingWeakest } = computeWeakestRule(
      context.allRules
