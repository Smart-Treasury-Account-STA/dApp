...
function record(
  overrides: Partial<ScheduledIntentRecord> = {}
...
describe('collectIntentIds', () => {
  it('reads the id out of an IntentCreated topic pair', () => {
    expect(collectIntentIds([{ topics: ['intent', bytes(ID_A)] }])).toEqual([
      ID_A,
    ])
  })
    ...
    const ids = collectIntentIds([
      {
        topics: [
          'init',
          'CCMPGTBAXHQX37QTXP2NGYFGL7NHG4CXQDJP43CWL3EQ5FVC36JODUDL',
        ],
      },
      {
        topics: [
          'execset',
          'GDWFIF3EMTDOUHV5UIKDARJU4ZI2TFGE6YBWHS4RDSCLZ7RVQO4XG3HV',
        ],
      },
  ...
  it('normalizes to lowercase hex, matching what makeIntentId emits', () => {
    ...
    expect(
      collectIntentIds([{ topics: ['intent', bytes(ID_A.toUpperCase())] }])
    ).toEqual([ID_A])
  })
...
describe('describeIntentStatus', () => {
  it('reports an unreadable intent as unknown rather than guessing', () => {
    expect(describeIntentStatus(record({ unreadable: true }), 150)).toBe(
      'unknown'
    )
  })
  ...
  it('reports cancellation before anything else', () => {
    ...
    expect(describeIntentStatus(record({ cancelled: true }), 150)).toBe(
      'cancelled'
    )
    expect(describeIntentStatus(record({ cancelled: true }), 900)).toBe(
      'cancelled'
    )
  })
  ...
  it('is active inside the window with executions left', () => {
    expect(
      describeIntentStatus(record({ executionCount: 1, maxExecutions: 3 }), 150)
    ).toBe('active')
  })
