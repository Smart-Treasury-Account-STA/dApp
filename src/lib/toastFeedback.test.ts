...
describe("buildToastFeedback", () => {
  it("maps a successful result to a success toast with no action", () => {
    const feedback = buildToastFeedback(
      { ok: true, title: "Payment confirmed", detail: "Transaction status: SUCCESS" },
      EXPLORER,
    );
  ...
  it("maps a failed result to an error toast", () => {
    const feedback = buildToastFeedback(
      { ok: false, title: "Schedule submission failed", detail: "Nonce has already been used." },
      EXPLORER,
    );
    ...
    const feedback = buildToastFeedback(
      {
        ...
        diagnostic: "HostError: Error(Object, InvalidInput)\n  0: [Diagnostic Event] …",
      },
      EXPLORER,
    );
  ...
  it("keeps the description to just the detail when there is no diagnostic", () => {
    ...
    expect(feedback.description).toBe("GABC…XYZ is connected on Stellar testnet.");
  });
});
