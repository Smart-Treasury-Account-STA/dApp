...
function sign(adminToken: string, expiresAt: number) {
  return createHmac("sha256", adminToken).update(`relayer-session:${expiresAt}`).digest("hex");
}

export function createSessionValue(adminToken: string, now = Math.floor(Date.now() / 1000)) {
