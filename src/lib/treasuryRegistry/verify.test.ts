...
describe("verifyTreasuryRegistration", () => {
  ...
  it("throws when the on-chain owner does not match the claimed owner", async () => {
    ...
    await expect(verifyTreasuryRegistration(record())).rejects.toThrow(/could not be verified/i);
  });
  ...
  ])("rejects a claim with a fabricated %s even when the owner is genuine", async (label, claim) => {
    loadSmartAccountLinksMock.mockResolvedValue(links());
  ...
  it("treats an absent on-chain address as a mismatch rather than a match", async () => {
    ...
    await expect(verifyTreasuryRegistration(record())).rejects.toThrow(/policy engine is unset/i);
  });
