...
describe('smart_account write descriptors', () => {
  ...
  it('builds add_context_rule with a Default context type and a single delegated signer', () => {
    const op = addContextRuleOperation({
      name: 'backup',
...
describe('policy_engine write descriptors', () => {
  it('routes policy writes through the source-account strategy', () => {
    const op = setRecipientAllowedOperation({
      recipient: SIGNER,
      allowed: true,
    })
  ...
  it('carries the operation symbol and the allowed flag', () => {
    const op = setOperationAllowedOperation({
      operation: 'transfer',
      allowed: false,
    })
...
describe('recovery_manager write descriptors', () => {
  ...
  it('respects a per-treasury contracts override, like every other write descriptor', () => {
    const customRecoveryManager =
      'CB4KZJ3I4XANE6GWPAMXCNXQ34PTQWPXVKFBBMLNKV25GAOXQC7RQUMS'
    const op = addGuardianOperation({
      guardian: SIGNER,
      contracts: {
        ...STELLAR_CONFIG.contracts,
        recoveryManager: customRecoveryManager,
      },
    })
