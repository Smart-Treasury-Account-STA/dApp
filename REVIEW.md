...
Full deployment record, including every WASM hash and every upload/create/initialize transaction: [`docs/TESTNET_DEPLOYMENT.md`](https://github.com/Smart-Treasury-Account-STA/smart-contracts/blob/v1-full-implementation/docs/TESTNET_DEPLOYMENT.md) in the `smart-contracts` repo.
...
The two live rejections shown on the public ledger above (§3, step 3) are read-only `validate_policy` simulations (`--send=no`). This isn't a gap in what was demonstrated; it's a consequence of how Soroban's transaction-preparation flow works, and it applies to every client, not just this one — including this dApp itself.
...
**This dApp hits the identical wall**, and for the same reason: `discoverTreasuryInvocation` and `prepareTransaction`, both in `src/lib/stellarClient.ts`, simulate the real call before ever asking a connected wallet to sign. A payment sized above the cap, an unlisted destination, a stale policy version, or a reused nonce all fail at that simulation step — which is exactly why the dApp can show you the rejection instantly (§ manual flow, step 5) without ever prompting Freighter/xBull for a signature. There's no way to get a wallet-signed, on-ledger rejection for these cases through this app, or through any Soroban client that follows the standard prepare-then-sign flow.

**Confirmed directly** against the live deployment, not just reasoned about: `scripts/execute_demo_transfer_payment.py` (in the `smart-contracts` repo) was run twice with a second, independently registered signer (`sta-signer`, delegated signer on context rule 1 — added after the original deployment, distinct from the founding deployer key on rule 0):
